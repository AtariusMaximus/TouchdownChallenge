...
Choose_Team_Colors_Loop
 ...
 ;──═◙ Plot Text
  ...
  if debounce=6 then gosub Reset_Game:gosub Set_Speed:goto start
...
Game_Event_GameOver_Loop
  if Time_Counter2=0 then goto Skip_Time_Count  ; Countdown from 9
  Time_Counter=Time_Counter+1                                            ; Pause after 4th Down before you can reset
  if Time_Counter=20 then Time_Counter2=Time_Counter2-1:Time_Counter=0   ; Increment Time_Counter2
Skip_Time_Count
 restorescreen                       ;Restore from savescreen
 if switchreset then reboot          ;Reset Game if you press Reset
